...
const OLLAMA_CHAT_URL =
  process.env.OLLAMA_CHAT_URL || "http://127.0.0.1:11434/api/chat";
...
router.post("/message", verifyToken, async (req, res) => {
  try {
    ...
    console.error("Ollama chatbot error:", error.response?.data || error.message);

    return res.status(500).json({
      success: false,
      message: "AI response failed. Make sure Ollama is running.",
      error: error.message,
    });
  }
});
...
router.post("/image", verifyToken, upload.single("image"), async (req, res) => {
  try {
    const userMessage =
      req.body.message?.trim() || "Analyze this image and explain what you see.";
    ...
    const botReply =
      ollamaResponse.data?.response || "Sorry, I could not analyze this image.";
    ...
    return res.status(500).json({
      success: false,
      message: "Image analysis failed. Make sure Ollama and llava are running.",
      error: error.message,
    });
  }
});
  ...
  try {
    ...
    const commentsResult = await pool.query(
      ...
      SELECT
        ...
        ticket_comments.sender_role,
        ticket_comments.created_at,
        users.full_name AS sender_name
      FROM ticket_comments
      LEFT JOIN users
      ON users.id = COALESCE(ticket_comments.admin_id, ticket_comments.user_id)
    ...
    console.error("Fetch customer ticket details error:", error.message);
...
router.post("/tickets/:ticketId/comment", verifyToken, async (req, res) => {
  try {
    const { ticketId } = req.params;
    ...
    const result = await pool.query(
      ...
      (ticket_id, user_id, comment, sender_role)
      VALUES ($1, $2, $3, $4)
      RETURNING id, comment, sender_role, created_at
      `,
      [ticketId, req.user.id, comment, "customer"]
    );
    ...
    console.error("Customer ticket reply error:", error.message);
...
router.post("/admin/tickets/:ticketId/comment", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `
      INSERT INTO ticket_comments
      (ticket_id, admin_id, comment, sender_role)
      VALUES ($1, $2, $3, $4)
      RETURNING id, comment, sender_role, created_at
      `,
      [ticketId, req.user.id, comment, "admin"]
    );
